...
with open(lda_model_path, 'rb') as lda_file:
    loaded_lda = pickle.load(lda_file)
...
def create_scatterplots(input_dir, output_dir):
    ...
    for week_num in range(25, 31):
        ...
        output_dir = f'encoded_data_{week}_lda'
        ...
        new_selected_features_scaled = loaded_lda.transform(selected_features_scaled)
        new_lda_df = pd.DataFrame(new_selected_features_scaled, columns=['x', 'y'])
        new_lda_df['Country_Region'] = country_region
        
        plt.figure(figsize=(16, 8))
        plt.scatter(new_lda_df['x'], new_lda_df['y'], c='blue', alpha=0.5)
        
        for i, row in new_lda_df.iterrows():
        ...
        scatterplot_save_path = os.path.join(scatterplot_save_dir, f'scatterplot_{week}_LDA.png')
