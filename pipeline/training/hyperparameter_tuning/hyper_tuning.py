...
def sdae_reduction(data_scaled, dim_num=2, hyperparameters=None):
    ...
    for dim in neurons_per_layer[:-1]:
        encoder_output = layers.Dense(
            dim, activation=activation_ch)(encoder_output)

    encoded_layer = layers.Dense(
        dim_num, activation=activation_ch)(encoder_output)
    ...
    for dim in reversed(neurons_per_layer[:-1]):
        decoder_output = layers.Dense(
            dim, activation=activation_ch)(decoder_output)
    decoder_output = layers.Dense(
        data_scaled.shape[1], activation='sigmoid')(decoder_output)
    ...
    history = autoencoder.fit(
        data_scaled, data_scaled, epochs=epoch_num, batch_size=batch_size_num, verbose=1)
    ...
    if best_hyperparameters is None or history.history['loss'][-1] < best_loss:
...
for hyperparameters in hyperparameter_grid:
    ...
    encoded_data = sdae_reduction(
        data_scaled, dim_num=2, hyperparameters=hyperparameters)
    print(f'Best Loss: {best_loss}')
    if best_hyperparameters:
        print(f'Best Hyperparameters: {best_hyperparameters}')

encoded_data, country_region = sdae_reduction(
    data_scaled, dim_num=2, hyperparameters=best_hyperparameters)

encoded_df = pd.DataFrame(encoded_data, columns=[
                          f'encoded_feature_{i}' for i in range(2)])
encoded_df['Country_Region'] = country_region

encoded_csv_directory = 'azure-ml-internal-project/pipeline/training/hyperparameter_tuning'
